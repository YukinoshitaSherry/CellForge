...
class GeneExpressionDataset(Dataset):
    def __init__(self, adata, perturbation_key='perturbation', scaler=None, pca_model=None, pca_dim=128, fit_pca=False, augment=False, is_train=True):
        ...
        print(f"{'Training' if is_train else 'Testing'} set perturbation dimension: {self.perturbations.shape[1]}")
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and self.training:
            ...
            mask_ratio = np.random.uniform(0.1, 0.3)
            ...
            if np.random.random() > 0.5:
                ...
                mix_idx = np.random.randint(0, len(self.expression_data))
                mix_x = self.expression_data[mix_idx]
                x = (1 - mix_ratio) * x + mix_ratio * mix_x
...
def train_model(model, train_loader, optimizer, scheduler, device, aux_weight=0.1):
    ...
    for i, batch in enumerate(train_loader):
        ...
        if hasattr(model, 'use_vae') and model.use_vae:
            vae_recon_loss = F.mse_loss(vae_recon, x)
            loss = mse_loss + aux_weight * pert_loss + vae_recon_loss + model.vae_beta * vae_kl
        else:
            loss = mse_loss + aux_weight * pert_loss
...
def objective(trial):
    ...
    params = {
        'pca_dim': 128,
        'n_hidden': trial.suggest_int('n_hidden', 512, 2048),
        ...
        'vae_latent_dim': trial.suggest_int('vae_latent_dim', 64, 128),
        'vae_hidden_dim': trial.suggest_int('vae_hidden_dim', 256, 512),
        'use_pert_emb': trial.suggest_categorical('use_pert_emb', [True, False]),
        'pert_emb_dim': trial.suggest_int('pert_emb_dim', 32, 64),
        'vae_beta': trial.suggest_float('vae_beta', 0.1, 1.0),
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f'Epoch {epoch+1}/{max_epochs}')):
            ...
            train_loss += loss.item() * params['gradient_accumulation_steps']
...
def main():
    ...
    print('Loading data...')
    train_path = "autodl-tmp/SrivatsanTrapnell2020_train_unseen.h5ad"
    test_path = "autodl-tmp/SrivatsanTrapnell2020_test_unseen.h5ad"
    ...
    train_adata.write_h5ad('processed_train_data.h5ad')
    test_adata.write_h5ad('processed_test_data.h5ad')
    
    
    print('Computing UMAP visualization...')
    
    combined_adata = train_adata.concatenate(test_adata)
    ...
    combined_adata.write_h5ad('combined_data_with_umap.h5ad')
    
    
    plt.figure(figsize=(12, 8))
    sc.pl.umap(combined_adata, color='perturbation', size=50, 
               title='UMAP visualization of gene expression data',
               save='umap_visualization.png')
